...
//tests changing of useState in ShowDocs making sure it's called to change the page view.
//parent state to change the page and ShowOne.
//tests is called correctly when a document item is clicked:
describe('ShowDocs Component', () => {
  ...
  beforeEach(() => {
      ...
      global.fetch = jest.fn(() =>
          Promise.resolve({
              ok: true,
              json: () => Promise.resolve({ data: [{ title: 'Document 1', id: 1 }, { title: 'Document 2', id: 2 }] }),
          })
      );
  });
  ...
  test('calls changeParentState checks if riht item clicked', async () => {
      ...
      expect(setSelectedItemMock).toHaveBeenCalledWith({ title: 'Document 1', id: 1 });
      expect(setSelectedItemMock).toHaveBeenCalledWith({ title: 'Document 2', id: 2 });
...
test('handles form input change', () => {
  ...
  fireEvent.change(contentInput, { target: { name: 'content', value: 'Updated Content' } });
...
test('submits the form and calls the backend', async () => {
  ...
  expect(fetch).toHaveBeenCalledWith("http://localhost:3001/posts/update_docs", {
    method: "POST",
    headers: {
